...
class player:
    ...
    def input(self):
        ...
        if TEST_STUFF:
            if keys[pygame.K_t]:
                levelzz[self.player_index] = TEST_LEVEL
                reset_rects(self.player_index)
                timer(True)
            if keys[pygame.K_b] and total_frames > self.last_KeyB + 10:
                self.last_KeyB = total_frames
                button_clickzz[self.player_index] += 1
                reset_rects(self.player_index, True)
                print(button_clickzz[self.player_index])
            if keys[pygame.K_z]:
                self.rect.y = -700
        if keys[pygame.K_r]:
            levelzz[self.player_index] = 0
            reset_rects(self.player_index)
            timer(True)
        if keys[pygame.K_l]:
    ...
    def movement(self):
        ...
        if self.x_speed < 0: self.x_speed += 0.5 * self.speed_mult
        colisions(self, rect_lists[self.player_index], False)
        ...
        colisions(self, rect_lists[self.player_index], True)
    ...
    def screen_side_check(self):
        ...
        if levelzz[self.player_index] != 30:
            if self.rect.top <= 0:
                self.rect.top = 0
                self.gravity = 0
        ...
        if levelzz[self.player_index] <= 22:
...
def game_funciton(scrolls):
    ...
    tmp = pygame.Surface((2400, 1200))
    tmp.set_alpha(int(200 / players))

    for _ in player_classes:
        rect_lists[_.player_index] = []
        x = 0
        y = 0
        for num in num_lists[_.player_index]:
            ...
            if  -50 < rect.centerx - scrolls[_.player_index][0] < 2650 and -50 < rect.centery - scrolls[_.player_index][1] < 1250: #optimization?
                if num == 0:
                    ...
                    if lava_hitbox_rect.colliderect(_.rect):
                        _.rect.topleft = 0, 0
                        _.gravity = 0
                        reset_rects(_.player_index)
                        print(f"death at {timer(False)} (by player {_.player_index})")
                        death_counter += 1
                        if SFX:
                            play_sound("death")
                    ...
                    if rect.colliderect(_.rect):
                        ...
                        print(f"button {button_clickzz[_.player_index]} hit at {timer(False)} in level {levelzz[_.player_index]} (by player {_.player_index})")
                        if SFX:
            ...
            if num == 8:
                ...
                if rect.colliderect(_.rect):
                    ...
                    button_clickzz[_.player_index] = 0
                    reset_rects(_.player_index)
                    if SFX:
...
def reset_rects(player_index: int, button=False):
    global ground_rect
    global sky_rect
    global end_rect
    global lava_rect
    ...
    if button == False:
        gravity_directions[player_index] = False
        if levelzz[player_index] == 30: player_classes[player_index].rect.topleft = (0, 3100)
        else: player_classes[player_index].rect.topleft = (50, 0)
